...
bool UFPS_CharacterMovementComponent::IsMovingSideways() const
{
	if (CharacterOwner)
	{
		...
		Right.Z = 0.0f;
		MoveDirection.Z = 0.0f;
		float DotProduct = FVector::DotProduct(Right, MoveDirection);
...
void UFPS_CharacterMovementComponent::OnMovementUpdated(float DeltaSeconds, const FVector& OldLocation,
...
void FSavedMove_FPSTemplate::SetMoveFor(ACharacter* C, float InDeltaTime, FVector const& NewAccel, FNetworkPredictionData_Client_Character& ClientData)
{
	...
	if (UFPS_CharacterMovementComponent* CustomMovementComponent = Cast<UFPS_CharacterMovementComponent>(C->GetCharacterMovement()))
	{
...
FNetworkPredictionData_Client_FPSTemplate::FNetworkPredictionData_Client_FPSTemplate(const UCharacterMovementComponent& ClientMovement)
