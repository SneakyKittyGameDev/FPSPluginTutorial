...
void UFPSTemplateStatics::SpawnProjectileImpactEffects(const AActor* WorldActor, const FHitResult& HitResult, const FDecalImpactSettings& DecalImpactSettings, const FSoundImpactSettings& SoundImpactSettings)
{
	if (UFPSTemplate_PhysicalMaterial* PhysMat = Cast<UFPSTemplate_PhysicalMaterial>(HitResult.PhysMaterial))
	{
		...
		ImpactEffects.DecalImpactSettings = DecalImpactSettings;
		ImpactEffects.SoundImpactSettings = SoundImpactSettings;
		SpawnEffects(WorldActor, HitResult, ImpactEffects);
	}
}

void UFPSTemplateStatics::SpawnFootstepImpactEffects(const AActor* WorldActor, const FHitResult& HitResult, const FDecalImpactSettings& DecalImpactSettings, const FSoundImpactSettings& SoundImpactSettings)
{
	if (UFPSTemplate_PhysicalMaterial* PhysMat = Cast<UFPSTemplate_PhysicalMaterial>(HitResult.PhysMaterial))
	{
...
void UFPSTemplateStatics::SpawnImpactEffect(const AActor* WorldActor, const FHitResult& HitResult, const FImpactEffects& ImpactEffect)
{
	SpawnEffects(WorldActor, HitResult, ImpactEffect);
}

void UFPSTemplateStatics::SpawnEffects(const AActor* WorldActor, const FHitResult& HitResult, const FImpactEffects& ImpactEffects)
{
	if (WorldActor)
	{
		if (UWorld* World = WorldActor->GetWorld())
		{
			const FSoundImpactSettings SoundImpact = ImpactEffects.SoundImpactSettings;
			if (USoundBase* ImpactSound = ImpactEffects.ImpactSound)
			{
				UGameplayStatics::SpawnSoundAtLocation(World, ImpactSound, HitResult.Location, FRotator::ZeroRotator, SoundImpact.VolumeMultiplier, SoundImpact.PitchMultiplier);
			}

			const FDecalImpactSettings DecalImpact = ImpactEffects.DecalImpactSettings;
